  ...
  <v-container class="pa-0">
    <v-card-title>
      <v-textarea
        ...
        v-model="comment"
        label="댓글 작성"
        placeholer="Ctrl + Enter로 댓글 작성 가능"
        outlined
        hide-details
        auto-grow
    ...
    <template v-for="(item, i) in items">
      <v-list-item :key="item.id">
        ...
        <v-list-item-content
          class="flex-nowrap flex-column flex-sm-row flex-md-row flex-lg-row"
        >
          ...
          <v-list-item-subtitle v-else class="comment">
            <v-textarea
              ...
              outlined
              label="댓글 수정"
              placeholder="Ctrl + Enter 를 누르면 저장됩니다"
        ...
        <v-list-item-action class="action-btn">
          ...
          <v-btn
            icon
            @click="remove(item)"
            v-if="(fireUser && fireUser.uid === item.uid) || (user && user.level === 0)"
          >
...
@media screen and (max-width: 798px) {
  ...
  .v-list-item > div:nth-child(1) {
    order: 2;
    margin: 0;
    flex: 0 0 50%;
  }
  .v-list-item > div:nth-child(2) {
